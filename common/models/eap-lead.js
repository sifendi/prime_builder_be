                        ...
                        (endAysnc)=>{
                            ...
                            Eaplead.app.dbConnection.execute(getTLH,[dataArrObj['lead_sub_district_id']],(err,resultObjArrs)=>{
                                ...
                                (endAysnc)=>{
                                    ...
                                    //console.log("dataArrObj['lead_sub_district_id']",dataArrObj['lead_sub_district_id']);
                                    Eaplead.app.dbConnection.execute(getAC,[dataArrObj['lead_sub_district_id']],(err,resultObjArrs)=>{
                                        async.each(resultObjArrs,(resultObj,callback)=>{
                                        	
											if((dataArrObj['lead_segment'] == 1 || dataArrObj['lead_segment'] == 2 || dataArrObj['lead_segment'] == 3 || dataArrObj['lead_segment'] == 4) && (dataArrObj['lead_support_ac']==1)){
                                        		allSuportUserListEmails.push({
	                                        		name:resultObj['realm'],
	                                        		email:resultObj['email'],
	                                        		type:'AC',
	                                        		mobile:resultObj['username'],
	                                        	});
                                        	}
                                        ...
                                        (endAysnc)=>{
                                            NotificationCenterModal.create(notificationArr,(err,models)=>{
												...
												if((dataArrObj['lead_segment'] == 1 || dataArrObj['lead_segment'] == 2 || dataArrObj['lead_segment'] == 3 || dataArrObj['lead_segment'] == 4) && (dataArrObj['lead_support_telesales']==1)){
													var cityId = dataArrObj['lead_city_id'];
													var querySupTelesales = `SELECT * FROM eap_support_contacts WHERE city_id > 0 and city_id=${cityId} and status=1 and support_type = 'TeleSalesBeton' `;
													Eaplead.app.dbConnection.execute(querySupTelesales,[],(err,resultObjTelDatas)=>{
														async.each(resultObjTelDatas,(resultObjTelData,callback2)=>{
															allSuportUserListEmails.push({
																name:resultObjTelData['name'],
																email:resultObjTelData['email'],
																type:resultObjTelData['support_type'],
																mobile:resultObjTelData['mobile'],
															});
															callback2();
														},(endAysnc2)=>{
												...
												if((dataArrObj['lead_support_ac']==1) && ((dataArrObj['lead_segment'] == 5) || (dataArrObj['lead_segment'] == 6))){
													var cityId = dataArrObj['lead_city_id'];
													var querySup = `SELECT * FROM eap_support_contacts WHERE city_id > 0 and city_id=${cityId} and status=1  `;
													if(dataArrObj['lead_segment'] == 5){
														querySup += ` and support_type = 'ASM'`;
													}else if(dataArrObj['lead_segment'] == 6){
														querySup += ` and support_type = 'KA'`;
													}
...
function emailNotifyLeadsSupport(fromUserData,dataObj,userEmailList){
	...
	//var timestamp = dataObj['lead_visit_date'];
	var date=new Date(dataObj['lead_visit_date']);
	var day = date.getDate(); // day part from the timestamp
	var month = (date.getMonth()+1); // month part from the timestamp
	...
	Eaplead.app.dbConnection.execute(sqlSegment,[dataObj['lead_segment']],(err,resultSegment)=>{
		if(resultSegment != ""){
			segmentName = resultSegment[0].segment_name;
			var fromEmail = 'support@primebuilder.id';
			var interviewDiscusn = "";
			if(dataObj['lead_interview_result']){
				interviewDiscusn = dataObj['lead_interview_result'].replace(/\n/g,"<br>");
			}
			var toEmail='kanad@experiencecommerce.com; dede.kusnawan@lafargeholcim.com; wahyudi.guntur@lafargeholcim.com; erick.setiawan@lafargeholcim.com;';
			//var toEmail='chirag.bodke@experiencecommerce.com; rahul.sadaye@experiencecommerce.com;';
			async.each(userEmailList,(userEmailD,callback1)=>{
				var sendFlag=false;
				//console.log("userEmailD['type']", userEmailD['type']);
				...
				if(sendFlag && userEmailD['email'] && userEmailD['email']!=''){
					var htmlTemplet=`
						...
						<p>Dear ${userEmailD['name']} (${userEmailD['mobile']}),<br/></p>

						<p>Mohon tindak lanjut pelanggan dengan detail sebagai berikut: <br/></p>
						<p>Nama : ${dataObj['lead_name']}</p>
						<p>Alamat : ${dataObj['lead_address']}</p>
						<p>Kota : ${dataObj['lead_city']}</p>
						<p>No. HP : ${dataObj['lead_mobile']}</p>
						<p>Segment : ${segmentName}</p>
						<p>Tanggal Kunjungan : ${formattedDate}</p>
						<p>Hasil Interview : ${interviewDiscusn}<br/></p>
						<br/>
						<p>Salam, <br/></p>
						<p>${fromUserData['name']} (${fromUserData['username']})</p>

						=========== End Indonesia Version =================
						<br/>
						=========== English Version =================

						<p>Dear ${userEmailD['name']},<br/><p>

						<p>Please follow up customer with detail as describe below: <br/></p>
						<p>Name : ${dataObj['lead_name']}</p>
						<p>Address : ${dataObj['lead_address']}</p>
						<p>City : ${dataObj['lead_city']}</p>
						<p>Mobile : ${dataObj['lead_mobile']}</p>
						<p>Segment : ${segmentName}</p>
						<p>Visit Date : ${formattedDate}</p>
						<p>Interview/Discussion Result : ${interviewDiscusn}<br/></p>
						<br/>
						<p>Best Regards, </p>
						<p>${fromUserData['name']} (${fromUserData['username']})</p>
					`;
					...
					//from: 'lhiprimebuilder@gmail.com',
					  from: fromEmail,
					  cc: fromUserData['email']?fromUserData['email']:'',
					  subject: 'Follow Up Duta Holcim Program untuk pelanggan '+dataObj['lead_name'],
					  text: '',
					  html: htmlTemplet
					}, function(err, mail) {
...
Eaplead.remoteMethod('addEditLead',{
	...
	Eaplead.getLeadCount = function(dataArrObj,cb){
		...
		var paramsArr = [];
		var DateFilCount = 0 ;
		var sqlQuery = "select count(el.lead_segment) as total from eap_lead el join eap_master_lead_segment es on el.lead_segment = es.segment_id join [User] u1 on el.created_by = u1.id left join eap_refer_customer rc on el.lead_refer_id=rc.refer_id and el.lead_refer_id > 0 left join  eap_lead SecEl on rc.refer_via = SecEl.lead_id left join eap_support_assignment esa on esa.lead_id = el.lead_id and esa.status=1 left join [User] us on us.id = esa.sph_id where 1=1 ";
		for(var o in dataArrObj) {
			if(o == "created_date"){
				if(dataArrObj[o]!=''){
					sqlQuery+=" AND el."+o+" >= (?)";
