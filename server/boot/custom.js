...
module.exports = function(app) {
    console.log('dyd js');
    app.APP_SECRET='dyd113';
	app.dbConnection = app.dataSources.accountDS.connector;
	app.dbConnectionString = {
        user: 'holcim',
        password: '=G|[7j?e',
        server: '35.154.240.171', 
        database: 'holcim_hpb' 
    };
...
app.cronFr = function(){
	...
	console.log("cronFr....");
		...
		app.dataSources.accountDS.connector.execute(sphQuery,null,(err,sphData)=>{
		//	.. console.log('sphData',sphData);
			if((sphData) && (sphData.length > 0)){
				
				async.each(sphData, function(sph, callback) {
					 console.log('sph C',sph);
														...
														(err)=>{
															async.each(queryResult, function(queryResultData, callback3) {
																...
																if(queryResultData.hpb_status == "maintain"){
																	var value = queryResultData.invoice_quantity*queryResultData.unit_value;
																...
																if(todayAchievemnt && (todayAchievemnt.length > 0)){
																	async.each(todayAchievemnt, function(queryTodayResultData, callback4) {
																		...
																		if(queryTodayResultData.hpb_status == "maintain"){
																			var value = queryTodayResultData.invoice_quantity*queryTodayResultData.unit_value;
									...
									async.each(targetForJson, function(tar, targetCallback) {
										...
										app.dataSources.accountDS.connector.execute(selectHouse,arr,(err,queryResult)=>{
											...
											if(tar.targetFor == "srku_house_num"){
												var houseNumRemaining = houseNumMonthly - houseNum;
												var todayTarget = Math.ceil(houseNumRemaining/totalDays);
												...
												srkuvol = Math.ceil(srkuvol/1000);
												srkuvolToday = Math.ceil(srkuvolToday/1000);
												var srkuVolRemaining = srkuVolMonthly - srkuvol;
												var todayTarget = Math.ceil(srkuVolRemaining/totalDays);
												var createdDate = Math.floor(Date.now());
												...
												maintain = Math.ceil(maintain/1000);
												maintainToday = Math.ceil(maintainToday/1000);
												var maintainRemaining = maintainMonthly - maintain;
												var todayTarget = Math.ceil(maintainRemaining/totalDays);
												var createdDate = Math.floor(Date.now());
												...
												switching = Math.ceil(switching/1000);
												switchingToday = Math.ceil(switchingToday/1000);
												var switchingRemaining = switchingMonthly - switching;
												var todayTarget = Math.ceil(switchingRemaining/totalDays);
												var createdDate = Math.floor(Date.now());
												...
												var hpbRemaining = hpbMonthly - hpb;
												var todayTarget = Math.ceil(hpbRemaining/totalDays);
												var createdDate = Math.floor(Date.now());
